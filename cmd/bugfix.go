...
var (
	...
	defaultBugTitleTemplate = "BUG {{.ID}} {{.Title}}"
)
...
func createBugfixCommand(ctx context.Context, bugID int, description string) error {
	...
	task, err := a.CreateTask(ctx, title, description, int(estimate), int(parentWorkItemID), relations, []string{"bugfix"}, parentUserStoryNamePattern)
